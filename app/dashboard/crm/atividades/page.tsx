...
export default async function AtividadesPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  const canCreate = hasPermission(profile.papel, "atividades.criar");
  const canEdit = hasPermission(profile.papel, "atividades.editar");
  ...
  let tasksQuery = supabase.from("tarefas").select(ACTIVITY_SELECT).eq("ativo", true);
  let leadsQuery = supabase.from("leads").select("id, nome").eq("status_operacional", "ativo");
  let atendimentosQuery = supabase.from("atendimentos").select("id, assunto");
  if (corretorPessoaId) {
    tasksQuery = tasksQuery.eq("responsavel_id", corretorPessoaId);
    leadsQuery = leadsQuery.eq("responsavel_id", corretorPessoaId);
    atendimentosQuery = atendimentosQuery.eq("responsavel_id", corretorPessoaId);
  }
  const [tasks, leads, atendimentos, negocios, imoveis, pessoas] = await Promise.all([
    tasksQuery.order("inicio_planejado_em", { ascending: true, nullsFirst: false }).order("created_at", { ascending: false }),
    leadsQuery.order("nome"),
    atendimentosQuery.order("created_at", { ascending: false }),
  ...
  return <main className="min-h-screen bg-[#F7F3ED] px-6 py-10 sm:px-8"><div className="mx-auto max-w-7xl">
    ...
    {canCreate || (canEdit && editing) ? <section className="mt-6"><ActivityForm key={editing?.id ?? "create"} options={options} activity={editing} /></section> : null}
