...
export default async function NegociosPage({ searchParams }: { searchParams: Promise<Record<string,string|string[]|undefined>> }) {
  ...
  return <main className="min-h-screen bg-[#F7F3ED] px-4 py-8 sm:px-8"><div className="mx-auto max-w-[1600px]">
    ...
    <div className="mt-8 grid gap-6 xl:grid-cols-2 2xl:grid-cols-3">{NEGOCIO_STAGES.map((stage)=>{const items=active.filter((item)=>item.etapa===stage.id);const total=items.reduce((sum,item)=>sum+(commercialValue(item)??0),0);const stalled=items.filter((item)=>daysInStage(item,now)>7).length;return <section key={stage.id} className="rounded-[2rem] border border-[#E8DDCB] bg-white p-5 shadow-sm"><div className="flex justify-between gap-4"><div><h2 className="text-xl font-semibold text-[#071E36]">{stage.label}</h2><p className="mt-1 text-xs text-[#64736D]">{stage.description}</p><p className="mt-3 text-sm font-semibold text-[#071E36]">{formatMoney(total,"BRL")}</p><p className={`mt-1 text-xs font-medium ${stalled>0?"text-amber-700":"text-[#64736D]"}`}>{stalled} {stalled===1?"negocio parado":"negocios parados"}</p></div><span className="h-fit rounded-full bg-[#C89B3C]/10 px-3 py-1 text-sm font-semibold text-[#8B6827]">{items.length}</span></div><div className="mt-4 grid gap-4">{items.length?items.map((item)=><NegocioCard key={item.id} item={item} partes={partesByNegocio.get(item.id)??[]} canEdit={canEdit} finalPermissions={finalPermissions} hasSuccessor={predecessorIds.has(item.id)} now={now}/>):<p className="rounded-2xl bg-[#F7F3ED] p-6 text-center text-sm text-[#64736D]">Nenhum Negocio nesta etapa.</p>}</div></section>})}</div>
