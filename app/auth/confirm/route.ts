...
export async function GET(request: NextRequest) {
  ...
  if (!tokenHash || !["invite", "recovery", "email"].includes(type ?? "")) return NextResponse.redirect(new URL("/login?error=invite", request.url));
  const { error } = await (await createClient()).auth.verifyOtp({ token_hash: tokenHash, type: type as "invite" | "recovery" | "email" });
